...
from utils.datasets import get_dataset_info, get_repetition_dataloader
from utils.fl_data_iter import data_iter_fedsgd
...
exp_params = {
    ...
    "fl_model_kwargs": {
        "fc_layers": [],
        "is_pretrain": False,  # 默认使用torch预训练参数
    ...
    "data_iter_kwargs": {
        ...
        "model_mode_train": True,  # 模型是否train()模式
        "dataloader_seed": 1,  # 数据加载器随机种子
        "device": "",  # 训练设备，留空则设为主设备
    },
    "rep_rate": 0.0,  # 标签重复率。0为不调整数据集顺序
...
def main(override_params: dict = {}):
    ...
    # fl model, dataset
    image_size = params["data_iter_kwargs"]["image_size"]
    ...
    if fl_model_kwargs["output_length"] < 1:
        dataset_info = get_dataset_info(params["data_iter_kwargs"]["dataset_name"])
    ...
    need_bn_statistics = "STG" in params["attacks"].keys()
    test_batchs = params["test_batchs"]
    dataloader = None
    if params["rep_rate"] > 0:
        dataloader = get_repetition_dataloader(
            name=params["data_iter_kwargs"]["dataset_name"],
            batch_size=params["data_iter_kwargs"]["batch_size"],
            shuffle=params["data_iter_kwargs"]["shuffle"],
            image_size=params["data_iter_kwargs"]["image_size"],
            train=params["data_iter_kwargs"]["train"],
            rep_rate=params["rep_rate"],
        )
    data_iter = data_iter_fedsgd(
        model=fl_model,
        dataloader=dataloader,
        need_bn_statistics=need_bn_statistics,
        **params["data_iter_kwargs"],
    )

    # 屏蔽指定模块
    model_mask = params["model_mask"]
    for k, v in model_mask.items():
        if v:
            fl_model.mask_modules(k)
    # attacks
    batch_size = params["data_iter_kwargs"]["batch_size"]
    ...
    recorder.add_extra(
        {
            ...
            "fl_model": params["fl_model"],
            "is_avgpool": params["fl_model_kwargs"]["is_avgpool"],
            "output_length": params["fl_model_kwargs"]["output_length"],
            "fc_layers": str(params["fl_model_kwargs"]["fc_layers"]),
            "fl_model_load_name": params["fl_model_load"]["load_name"],
            "model_mask": str(model_mask),
            "test_batchs": test_batchs,
            "seed": params["seed"],
            "dataloader_seed": params["data_iter_kwargs"]["dataloader_seed"],
        }
    )
    ...
    for data in data_iter:
        ...
        logger.info(f"第{test_num}次实验：FedSGD 训练结束，开始模拟攻击")
        for atk_name, atk in attacks.items():
            logger.info(f"攻击 {atk_name} 开始")
            assess_info = atk.assess(fl_model, batch_size, image_size, data)
